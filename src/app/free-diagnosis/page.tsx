        ...
        <div className="container mx-auto px-4 py-14 md:py-20">
          <div className="mx-auto max-w-5xl">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {type ? (
                <Badge className="rounded-full px-3 py-1">{config?.pill}</Badge>
              ) : (
                <Badge className="rounded-full px-3 py-1">Kies je route</Badge>
              )}
              ...
              <span className="ml-auto hidden sm:flex items-center gap-2 rounded-full border border-border/70 bg-background/30 px-3 py-1.5 text-sm text-muted-foreground">
                <Clock className="h-4 w-4" /> {type ? config?.estimate : "~1 min"}
              </span>
            </div>
            ...
            <h1 className="mt-6 font-headline text-4xl md:text-6xl font-bold tracking-tight">
              {type ? (
                ...
                <>
                  Gratis diagnose{" "}
                  <span className="text-muted-foreground">(kies één route, dan houden we het rustig)</span>
                </>
              )}
            </h1>
            ...
            <p className="mt-4 max-w-3xl text-base md:text-xl text-muted-foreground leading-relaxed">
              {type
                ? config?.subtitle
                : "Je site hoeft niet alles tegelijk te zijn. Jij kiest waar je hulp mee wilt, ik diagnoseer de bottleneck en geef je een duidelijke volgende stap."}
            </p>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              {type ? (
                ...
                >
                  {config?.primaryCta} <ArrowRight className="ml-2 h-4 w-4" />
                </a>
              ) : (
                <a
                  href="#choose"
                  className={cn(buttonVariants({ size: "lg" }), "font-semibold")}
                >
                  Kies je diagnose <ArrowRight className="ml-2 h-4 w-4" />
                </a>
              )}
